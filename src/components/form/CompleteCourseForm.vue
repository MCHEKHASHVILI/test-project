...
<template>
  <div v-if="!!getCourseDetailed" class="w-full flex flex-col justify-between gap-5.5">
    ...
    >
      <div
        v-if="getCourseDetailed.enrollment.progress < 100"
        class="w-fit p-4 bg-[#736BEA1A] rounded-full"
      >
    ...
    <div class="flex flex-row items-center gap-2 justify-start">
      ...
      <small
        class="text-xl font-medium text-grayscale-600"
    ...
    <div class="flex flex-row items-center gap-2 justify-start">
      ...
      <small
        class="text-xl font-medium text-grayscale-600 capitalize"
    ...
    <div class="flex flex-col gap-12">
      <div class="flex flex-col w-full gap-3 items-start justify-between">
        ...
        <div class="w-full bg-purple-100 rounded-full h-6">
          <div
            class="bg-purple-500 h-6 rounded-[30px] transform duration-300 transition-all"
            :style="{ width: `${getCourseDetailed.enrollment.progress}%` }"
          ></div>
        </div>
      </div>
      <button
    ...
    <div class="flex flex-col items-center gap-4.5 py-10 px-12.5 rounded-lg bg-grayscale-50">
      <span
        class="text-grayscale-600 text-[16px] font-medium leading-6 lowercase first-letter:capitalize"
        v-text="
