...
<template>
  <div class="flex flex-col gap-2.75">
    <TogglePanel v-for="item in panels" :panel="item" :key="item.id" @toggle="togglePanel">
      <ScheduleBox
        v-if="item.id === 1 && scheduleIds"
        v-model="scheduleId"
        :available-ids="scheduleIds"
        :options="updatedScheduleOptions"
      />
      <TimeBox
        v-if="item.id === 2 && timeSlotIds"
        v-model="timeSlotId"
        :available-ids="timeSlotIds"
        :options="updatedTimeSlotOptions"
      />
      <SessionBox
        v-if="item.id === 3 && sessionTypeIds"
        class="flex-1 flex flex-row justify-between gap-2"
        v-model="sessionTypeId"
        :available-ids="sessionTypeIds"
        :options="updatedSessionOptions"
      />
    </TogglePanel>
    <div
      class="w-full flex flex-col gap-7.75 border border-grayscale-100 rounded-xl bg-gray-50 p-10"
    >
      ...
      <button
        ...
        :disabled="!courseOrder.bookingAuthorized"
        ...
        >
          <div div class="flex flex-col gap-2.25">
            <div class="flex flex-row gap-1.5">
              ...
              <span
                class="capitalize text-md text-grayscale-800 font-medium leading-6"
                v-text="'authentication required'"
              />
            </div>
            <span
              class="lowercase first-letter:capitalize text-grayscale-400 text-xs"
