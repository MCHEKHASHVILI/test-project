...
watch(() => props.actualRating, (val) => { model.value = val }, { immediate: true })
...
<template>
    <div class="flex items-center gap-1" role="radiogroup" aria-label="Star rating">
        <label v-for="star in maxRating" :key="star" class="cursor-pointer relative block"
            :style="{ width: `${width}px`, height: `${height}px` }" @mouseenter="hoveredRating = star"
            @mouseleave="hoveredRating = null">
            <input type="radio" :name="radioGroupName" :value="star" v-model="model" class="sr-only"
                :aria-label="`${star} star${star !== 1 ? 's' : ''}`" />

            <svg :viewBox="viewBox" :width="width" :height="height" xmlns="http://www.w3.org/2000/svg"
                class="transition-colors duration-150" aria-hidden="true">
                <defs>
                ...
                <path v-for="(path, i) in svgPaths" :key="i" :d="path.d" :fill="getFillColor(star)"
                    :fill-rule="path.fillRule" :clip-rule="path.clipRule" />
            </svg>
        </label>
    </div>
</template>
