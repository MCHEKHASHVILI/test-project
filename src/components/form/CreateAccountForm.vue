...
const { isLoading, getErrors } = storeToRefs(useApiStore())
const {
  currentStep,
  totalSteps,
  email,
...
<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="w-full flex justify-between items-center gap-2">
      <div v-for="i in totalSteps" :key="i" class="w-full h-2 rounded-[30px] transition-colors duration-300 ease-in-out"
    ...
    <form class="modal" @submit.prevent>
      ...
      <div v-else-if="currentStep === 2" class="flex flex-col gap-6">
        <TextInput type="text" label="Password*" autocomplete="new-password" icon="eye_open" v-model="password"
          :errors="getErrors?.password" />
        <TextInput type="password" label="Confirm Password*" autocomplete="new-password" icon="eye_closed"
          v-model="passwordConfirmation" :errors="getErrors?.password" />
      </div>

      <!-- Step 3 -->
      <div v-else-if="currentStep === 3" class="flex flex-col gap-6">
        <TextInput type="text" label="UserName*" v-model="userName" :errors="getErrors?.username" />
      ...
      <div class="mt-8 flex justify-between">
        <button v-if="currentStep < totalSteps" @click.prevent="registrationStore.nextStep"
          class="w-full btn-primary p-2.5">
          Next
        </button>
        <button v-else class="w-full btn-primary p-2.5" @click="$emit('submit')">
          Create Account
        </button>
      </div>
    </form>
