  ...
  >
    ...
    <div class="flex flex-col space-y-6 text-center">
      <p class="text-xl font-medium text-grayscale-700 leading-[100%]">
        ...
        <span class="font-semibold leading-6">“{{ conflicts.conflictingCourseName }}”</span> with
        the same schedule: <span class="font-semibold leading-6">{{ conflicts.schedule }}</span>
      </p>
      <div class="w-full flex flex-row items-center justify-between gap-2">
        <button
          class="btn-outline btn-outline-sm w-1/2 text-normal text-nowrap capitalize p-3"
          @click.prevent="() => enrollCourse(true)"
        >
          continue anyway
        </button>
        <button
          class="btn-primary btn-primary-sm w-1/2 text-normal text-nowrap capitalize p-3"
