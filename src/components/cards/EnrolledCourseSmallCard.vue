    ...
    <div v-if="data?.course || !fallback" class="card card-progress flex flex-col gap-2">
        <div class="flex flex-row items-start justify-between gap-4">
            ...
            <div class="w-full flex flex-col justify-between gap-2.25">
                <div class="flex flex-row items-center justify-between">
                    <small class="text-body-xs text-grayscale-500 capitalize text-nowrap"
        ...
        <div class="flex flex-row items-end justify-between">
            <div class="w-84 flex flex-col gap-1 items-start justify-between pb-1">
                <span class="text-helper-medium tracking-normal text-grayscale-900 capitalize"
                    v-text="`${data.progress}% Completed`" />
                <div class="w-full bg-purple-100 rounded-full h-3.75">
                    <div class="bg-purple-500 h-3.75 rounded-[30px] transform duration-300 transition-all ease-out"
                        :style="{ width: `${data.progress}%` }"></div>
                </div>
            </div>
            <AppLink :to="{ name: 'details', params: { id: data.course.id } }">
                <button class="btn-outline text-button-s text-nowrap">
                    View
                </button>
            </AppLink>
        </div>
    ...
    <div v-else class="card card-progress flex flex-col gap-2">
        ...
        <div class="flex flex-row items-start justify-between gap-4">
            ...
            <div class="w-full flex flex-col justify-between gap-2.25">
                ...
                <h4 class="text-heading-4 leading-6 text-grayscale-900"
