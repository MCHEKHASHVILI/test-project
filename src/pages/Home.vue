...
<template>
  ...
  <div class="relative flex flex-col">
    <section v-if="isAuthenticated && getCoursesEnrolled.slice(0, 3).length" name="continue-learning"
    ...
    <section v-if="getCoursesFeatured" name="featured-courses" class="wrapper mt-20 flex flex-col gap-8">
      <div class="flex flex-col justify-between gap-1.5">
        ...
        <h2 class="text-body-m font-normal leading-6.5 text-grayscale-700">Choose from our most popular courses and
          begin
          your journey
        </h2>
      </div>
    ...
    <section v-if="!isAuthenticated" name="continue-learning-fallback" class="wrapper mt-20 flex flex-col gap-8">
      <div class="flex flex-col justify-between gap-1.5">
        <h1 class="text-heading-1 tracking-normal text-grayscale-950">Continue Learning</h1>
        <div class="flex flex-row w-full items-center justify-between">
          <h2 class="text-body-m text-grayscale-700" v-text="'Pick up where you left'" />
      ...
      <div class="relative grid grid-cols-3 justify-between gap-6">
        <div class="absolute inset-0 flex flex-col items-center justify-center rounded-xl backdrop-blur-[20px]">
          <div class="h-full w-full flex items-center justify-center">
            <div
              ...
              <AppLink :to="router.resolve({ name: 'action.modal', params: { name: 'LogInModal' } })">
                <button class="btn-primary text-button-s capitalize" v-text="'log in'" />
              </AppLink>
            </div>
          </div>
        </div>
        <EnrolledCourseSmallCard v-for="course in 3" :key="course" :data="{} as CourseEnrolled" :fallback="true" />
      </div>
    </section>
  </div>
</template>
